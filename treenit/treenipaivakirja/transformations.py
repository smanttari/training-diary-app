...
from treenipaivakirja.utils import duration_to_string, dataframe_to_dict,coalesce,parse_coordinates_from_gpx,remove_decimals
...
def trainings_to_df(user_id, columns, startdate=None, enddate=None, sport='Kaikki', restdays=True, duration_format='str', date_format='%Y-%m-%d'):
    ...
    days_df = pd.DataFrame(days, dtype=str, columns=['Pvm','vvvvkkpp','Viikonpäivä','Vko'])
    ...
    trainings_df = pd.DataFrame(trainings, dtype=str, columns=[
    ...
    trainings_df[['h','min']] = trainings_df[['h','min']].astype(float)
    ...
    trainings_df['Keskisyke'] = trainings_df.apply(lambda row: remove_decimals(row['Keskisyke']), axis=1)
    trainings_df['Tuntuma'] = trainings_df.apply(lambda row: remove_decimals(row['Tuntuma']), axis=1)
    ...
    if zones:
        zones_df = pd.DataFrame(zones, dtype=str, columns=['id','teho','kesto','h','min'])
        zones_df[['kesto','h','min']] = zones_df[['kesto','h','min']].astype(float)
...
def trainings_base_to_df(user_id):
    ...
    trainings_df['laji_ryhma'] = trainings_df['laji_ryhma'].astype('object')
...
def trainings_per_season_to_df(trainings_df):
    trainings = trainings_df[['kausi','kesto', 'matka']].groupby('kausi').sum().reset_index()
    trainings[['kesto','matka']] = trainings[['kesto','matka']].round(0)
    return trainings


def trainings_per_year_to_df(trainings_df):
    trainings_per_year = trainings_df[['vuosi','kesto', 'matka']].groupby('vuosi').sum().reset_index()
...
def trainings_per_month_to_df(trainings_df,user_id):
    ...
    df = trainings_df[['vuosi','kk','kesto','matka']].groupby(['vuosi','kk']).sum().reset_index()
...
def trainings_per_week_to_df(trainings_df,user_id):
    ...
    df = trainings_df[['vuosi','vko','kesto','matka']].groupby(['vuosi','vko']).sum().reset_index()
...
def hours_per_sport_to_json(trainings_df):
    hours = trainings_df[['vuosi','laji_nimi','kesto']].groupby(['vuosi','laji_nimi']).sum().reset_index().round(1)
...
def hours_per_sport_group_to_json(trainings_df):
    hours = trainings_df[['vuosi','laji_ryhma','kesto']].groupby(['vuosi','laji_ryhma']).sum().reset_index().round(1)
